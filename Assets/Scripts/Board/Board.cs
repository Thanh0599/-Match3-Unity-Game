...
public class Board
{
    ...
    List<NormalItem.eNormalType> _listNeighbourTypes = new List<NormalItem.eNormalType>();
    NormalItem neightbourNormalItem = null;
    internal void FillGapsWithNewItems()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                _listNeighbourTypes.Clear();
                CheckNeighbourCellType(cell.NeighbourBottom);
                CheckNeighbourCellType(cell.NeighbourUp);
                CheckNeighbourCellType(cell.NeighbourLeft);
                CheckNeighbourCellType(cell.NeighbourRight);

                NormalItem.eNormalType minType = GetMinType();
                if (_listNeighbourTypes.Contains(minType))
                {
                    minType = Utils.GetRandomNormalTypeExcept(_listNeighbourTypes.ToArray());
                }
    ...
    private void CheckNeighbourCellType(Cell neighbourCell)
    {     
        ...
        {
            neightbourNormalItem = neighbourCell.Item as NormalItem;
            if (neightbourNormalItem != null)
            {
                _listNeighbourTypes.Add(neightbourNormalItem.ItemType);
            }
        }
    }

    private NormalItem.eNormalType GetMinType()
    {
        ...
        List<NormalItem> listNormalItem = cells.Select(x => x.Item as NormalItem).ToList();
        return listNormalItem.GroupBy(x => x.ItemType).OrderBy(x => x.Count()).FirstOrDefault().Key;
    }
